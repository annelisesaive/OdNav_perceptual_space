...
Example of an iterative PCA followed by a PCA

```{r}
filename <- paste(path1,"evals_su=S01-BCS_stim=odor_scaled.csv",sep = "")
...
#Find the subset of points on the convex hull of data
hpts <- chull(pca_df[4:5])
hpts <- c(hpts, hpts[1])
{plot(pca_df[4:5], cex = 0.5)
pts <- pca_df[4:5][hpts,]
lines(pts)}

#Create polygon & compute surface
chull.poly <- Polygon(pts, hole=F)
chull.area <- chull.poly@area
sum_df['surf poly'] <- chull.area #summary(res.PCA)

#Compupte mean dist and circle area
...
Check the PCA projections for variables and individuals

```{r}
plot.PCA(res.PCA, choix = "var", habillage = 'cos2', title = " ")
plot.PCA(res.PCA, col.ind="white", col.quali="blue")
```

We now do the same thing for all subjects and for all sensory modalities:
